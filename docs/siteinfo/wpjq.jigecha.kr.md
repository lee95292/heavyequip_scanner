...
- Origin: `http://wpjq.jigecha.kr`
- 사이트 내 매물 수: 약 `7,392`건
- 확인한 대표 매물 URL: `http://wpjq.jigecha.kr/kwa-sell_jigecha`
- 취급장비 정보: jigecha.kr과 같은 지게차코리아 매물 체계를 사용합니다. 중고 지게차, 디젤/전동 지게차, 부품/타이어/배터리 관련 메뉴 확인.
- 대표자/운영자 정보: 원본 CSV 연결 업체는 청룡중장비매매상사입니다. 대표전화 041-855-6585, 취급품목은 굴삭기/지게차/덤프트럭 매입·매매로 기록되어 있습니다.
- 연결된 원본 업체: 청룡중장비매매상사
- 원본 CSV 대표전화: 041-855-6585

## API 정보

- API 성격: JSON API가 아니라 `HTML 목록 엔드포인트`입니다. `requests + BeautifulSoup` 방식으로 목록 HTML을 받아 매물 행과 상세 URL을 파싱하는 구조입니다.
- 단일 API 엔드포인트: `http://wpjq.jigecha.kr/kwa-sell_jigecha`
...
- 예시 요청: `http://wpjq.jigecha.kr/kwa-sell_jigecha?PB_1483420906=2`
